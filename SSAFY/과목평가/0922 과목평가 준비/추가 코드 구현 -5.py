...
# 경로 탐색 알고리즘
def bfs(grid, start, target, wall, water, init_ammo):
    rows, cols = len(grid), len(grid[0])
    queue = deque([(start, [], init_ammo)])
    visited = {start, init_ammo}

    while queue:
        (r, c), actions, ammo = queue.popleft()
        ...
        for d, (dr, dc) in enumerate(DIRS):
            ...
            if cell in ("G,", "X") and ((nr, nc), ammo) not in visited:
                visited.add(((nr, nc), ammo))
                queue.append(((nr, nc), actions + [MOVE_CMDS[d]], ammo))

            elif cell == 'T' and ammo > 0 and ((nr, nc), ammo -1) not in visited:
                visited.add(((nr, nc), ammo-1))
                queue.append(((nr, nc), actions + [FIRE_CMDS[d], MOVE_CMDS[d]], ammo-1))
...
# 최초 경로 탐색
init_ammo = int(my_allies['M'][2]) if 'M' in my_allies else 0
actions = bfs(map_data, start, target, WALL_SYMBOL, WATER_SYMBOL, init_ammo)
