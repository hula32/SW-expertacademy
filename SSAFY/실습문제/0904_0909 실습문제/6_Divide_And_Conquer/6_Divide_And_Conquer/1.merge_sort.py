...
def merge_sort(li): 
    ...
    # 1. 재귀 호출하기 전에 절반 씩 분할
    mid = len(li) // 2
    left = li[:mid]    # 리스트의 앞쪽 절반
    right = li[mid:]   # 리스트의 뒤쪽 절반

    # 길이가 1이 될 때까지 분할
    left_list = merge_sort(left) # Left subtree
    right_list = merge_sort(right) # Right subtree

    # print(left_list, right_list)
    # 분할이 완료되면
    # 2. 병합
    merged_list = merge(left_list, right_list) # V : Left, Right subtree 합치기
    return merged_list
